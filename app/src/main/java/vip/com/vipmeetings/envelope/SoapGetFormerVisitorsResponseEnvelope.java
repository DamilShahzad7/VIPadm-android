package vip.com.vipmeetings.envelope;


import org.simpleframework.xml.Element;
import org.simpleframework.xml.Namespace;
import org.simpleframework.xml.NamespaceList;
import org.simpleframework.xml.Root;

import vip.com.vipmeetings.body.AddDeviceTokenResponseBody;
import vip.com.vipmeetings.body.GetFormerVisitorsRequestBody;
import vip.com.vipmeetings.body.GetFormersResponseBody;
import vip.com.vipmeetings.response.GetFormerVisitorsResponse;

@Root(name = "soap12:Envelope", strict = false)
@NamespaceList({

        @Namespace(prefix = "xsi", reference = "http://www.w3.org/2001/XMLSchema-instance"),
        @Namespace(prefix = "xsd", reference = "http://www.w3.org/2001/XMLSchema"),
        @Namespace(prefix = "soap12", reference = "http://schemas.xmlsoap.org/soap/envelope/")
})
public class SoapGetFormerVisitorsResponseEnvelope {

    @Element(name = "Body", required = false)
    private GetFormersResponseBody body;

    public GetFormersResponseBody getBody() {
        return body;
    }

    public void setBody(GetFormersResponseBody body) {
        this.body = body;
    }
}
